...
int main(int argc, char** argv)
{
  ...
  visual_tools.publishText(text_pose, "MoveGroupInterface Exercise", rvt::WHITE, rvt::XLARGE);
  visual_tools.trigger();

  ROS_INFO_NAMED("exerciseB2", "Planning frame: %s", move_group_interface.getPlanningFrame().c_str());
  ROS_INFO_NAMED("exerciseB2", "End effector link: %s", move_group_interface.getEndEffectorLink().c_str());
  ROS_INFO_NAMED("exerciseB2", "Available Planning Groups:");
  ...
  ROS_INFO_NAMED("exerciseB2", "going to home position %s", success ? "" : "FAILED");
  visual_tools.deleteAllMarkers();
  visual_tools.trigger();
  ...
  ROS_INFO_NAMED("exerciseB2", "C: Command (plan and optionally execute) the robot to move to a specific pose %s", success ? "" : "FAILED");

  visual_tools.publishAxisLabeled(target_pose, "C: Pose 1");
  ...
  move_group_interface.setJointValueTarget(joint_group_positions);
  success = (move_group_interface.plan(my_plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);
  ROS_INFO_NAMED("exerciseB2", "D: Command (plan and optionally execute) the robot to move to a specific joint state %s", success ? "" : "FAILED");
  visual_tools.deleteAllMarkers();
  visual_tools.publishText(text_pose, "Trajectory in joint space", rvt::WHITE, rvt::XLARGE);
  visual_tools.publishTrajectoryLine(my_plan.trajectory_, joint_model_group);
  visual_tools.trigger();
  visual_tools.prompt("Press 'next' to move to the shown pose");
  move_group_interface.execute(my_plan);
  ...
  success = (move_group_interface.plan(my_plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);
  ROS_INFO_NAMED("exerciseB2", "E: Command (plan and optionally execute) the robot to follow a linear path between two Cartesian poses %s", success ? "" : "FAILED");
 ...
 //F: Command (plan and optionally execute) the robot to move along a trajectory defined by a set of waypoints
  ...
  ROS_INFO_NAMED("exerciseB2", "F: Command (plan and optionally execute) the robot to move along a trajectory defined by a set of waypoints (%.2f%% acheived)", fraction * 100.0);
  ...
  //G: Command (plan and optionally execute) the robot to move along a trajectory (side length 0.2 m) parallel to X-Y world plane and with Z coordinate 0.3
  current_pose = move_group_interface.getCurrentPose().pose;
  ...
  ROS_INFO_NAMED("exerciseB2", "G, 0.3 parallel (%.2f%% acheived)", fraction * 100.0);
  visual_tools.deleteAllMarkers();
  visual_tools.publishText(text_pose, "Cartesian Path", rvt::WHITE, rvt::XLARGE);
  visual_tools.publishPath(waypoints, rvt::LIME_GREEN, rvt::SMALL);
  for (std::size_t i = 0; i < waypoints.size(); ++i)
  ...
  */
  ...
  ROS_INFO_NAMED("ExerciseB2", "Add an object into the world");
  ...
  cylinder_primitive.dimensions[primitive.CYLINDER_HEIGHT] = 0.30;
  cylinder_primitive.dimensions[primitive.CYLINDER_RADIUS] = 0.05;

  // We define the frame/pose for this cylinder so that it appears in the gripper
  object_to_attach.header.frame_id = move_group_interface.getEndEffectorLink();
  geometry_msgs::Pose grab_pose;
  grab_pose.orientation.z = -0.701;
  grab_pose.orientation.w = -0.701;
  ...
  ROS_INFO_NAMED("ExerciseB2", "Attach the object to the robot");
